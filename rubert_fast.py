...
def main():
    ...
    def load_dataset_fast():
        try:
            ...
            if len(dataset["train"]) > max_train_samples:
                print(
                    f"Ограничиваем train с {len(dataset['train'])} до {max_train_samples} примеров"
                )
            ...
            if (
                ...
                if len(dataset[eval_split]) > max_eval_samples:
                    print(
                        f"Ограничиваем {eval_split} с {len(dataset[eval_split])} до {max_eval_samples} примеров"
                    )
            ...
            print(
                f"Создан тестовый датасет: {len(dataset['train'])} train, {len(dataset['validation'])} validation"
            )
    ...
    def convert_labels_fast(example):
        ...
        if isinstance(sentiment, (int, float)):
            ...
            if any(
                ...
                for word in ["negative", "негатив", "neg", "плох", "ужас"]
            ):
    ...
    print(
        f" Данные готовы: {len(tokenized_datasets['train'])} train, {len(tokenized_datasets[eval_split])} eval"
    )
    ...
    for i, result in enumerate(results, 1):
        emoji = (
            ...
            else "😐" if result["sentiment"] == "NEUTRAL" else "😞"
        )
        print(
            f"{i}. {emoji} {result['sentiment']:8} ({result['confidence']:.3f}) | {result['text'][:50]}..."
        )
