...
class ModelTester:
    ...
    def setup_models(self):
        ...
        self.models = {
            "1": {
                "name": " Наша обученная RuBERT модель версия: tiny",
                "path": "./trained-fast-sentiment-model1",  # Путь к нашей обученной модели
                "type": "local",
            },
            "2": {
                "name": " Наша обученная RuBERT модель версия: base",
                "path": "./trained-base-sentiment-model",  # Путь к нашей обученной модели
                "type": "local",
            },
            # "3": {
        ...
        try:
            ...
            for i, text in enumerate(texts):
                ...
                emoji = (
                    ...
                    else "😐" if sentiment == "NEUTRAL" else "😞"
                )
    ...
    def test_complex_sentences(self):
        ...
        test_sentences = [
            #  ПРОСТЫЕ И ЯСНЫЕ
            {
                "text": "Это просто великолепно! Очень доволен покупкой!",
                "expected": "POSITIVE",
            },
            {
                "text": "Ужасное качество, никогда больше не куплю!",
                "expected": "NEGATIVE",
            },
            {"text": "Нормальный товар, ничего особенного.", "expected": "NEUTRAL"},
            #  СРЕДНЕЙ СЛОЖНОСТИ
            {
                "text": "В целом неплохо, но есть небольшие недочеты.",
                "expected": "NEUTRAL",
            },
            {
                "text": "Отличный продукт, жаль только что дороговато.",
                "expected": "POSITIVE",
            },
            {
                "text": "Не сказать что плохо, но и хорошего мало.",
                "expected": "NEGATIVE",
            },
            #  СЛОЖНЫЕ И ПРОТИВОРЕЧИВЫЕ
            {
                "text": "Ну просто замечательно... если не смотреть на цену.",
                "expected": "NEUTRAL",
            },
            {
                "text": "Качество отличное, но обслуживание подвело.",
                "expected": "NEUTRAL",
            },
            {
                "text": "Прекрасный сервис! Жаль, что товар не оправдал ожиданий.",
                "expected": "NEGATIVE",
            },
            #  САРКАЗМ И ИРОНИЯ
            {
                "text": "О да, просто восхитительно... ждать месяц за доставку.",
                "expected": "NEGATIVE",
            },
            {
                "text": "Ну конечно, отличное качество - сломалось через день.",
                "expected": "NEGATIVE",
            },
            {
                "text": "Просто супер, если вас устраивает низкое качество.",
                "expected": "NEGATIVE",
            },
            #  ДЛИННЫЕ И СЛОЖНЫЕ
            {
                "text": "Несмотря на некоторые недостатки в сборке, общее впечатление от продукта остается положительным, поскольку основные функции работают стабильно и соответствуют заявленным характеристикам.",
                "expected": "POSITIVE",
            },
            {
                "text": "Хотя дизайн продукта действительно привлекательный и эргономичный, постоянные проблемы с программным обеспечением и низкая надежность компонентов существенно снижают общую оценку и заставляют сомневаться в целесообразности покупки.",
                "expected": "NEGATIVE",
            },
            #  НЕОПРЕДЕЛЕННЫЕ
            ...
            {
                "text": "С одной стороны хорошо, с другой стороны плохо.",
                "expected": "NEUTRAL",
            },
        ]
        ...
        for i, (result, expected_sentiment) in enumerate(zip(results, expected)):
            ...
            # Вывод результата
            print(
                f"{i+1:2d}. {match_icon} {result['emoji']} {result['confidence_color']}"
            )
            print(
                f"    Предсказано: {result['sentiment']:8} (уверенность: {result['confidence']:.3f})"
            )
    ...
    def interactive_test(self):
        ...
        while True:
            ...
            if results:
                ...
                print(
                    f"   {result['confidence_color']} Уверенность: {result['confidence']:.3f}"
                )
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            for model_key in tester.models.keys():
                if tester.load_model(model_key):
                    results = tester.predict_sentiment(
                        ["Отличный продукт!", "Ужасное качество", "Нормально"]
                    )
                    if results:
            ...
            for model_key, result in sorted(
                ...
                print(
                    f"   {result['name']}: {result['avg_confidence']:.3f} ср. уверенность"
                )
